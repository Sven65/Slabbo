...
public class MySQLStore implements DataStore {
    ...
    Slabbo.getInstance().getConfig().getString("mysql.host") +
    ":" +
    Slabbo.getInstance().getConfig().getString("mysql.port") +
    "/" +
    Slabbo.getInstance().getConfig().getString("mysql.database") +
    "?usessl=" +
    Slabbo.getInstance().getConfig().getBoolean("mysql.useSSL") +
    "&verifyServerCertificate=" +
    Slabbo.getInstance().getConfig().getBoolean("mysql.verifyCertificate") +
    "&allowPublicKeyRetrieval=" +
    Slabbo.getInstance().getConfig().getBoolean("mysql.allowPubKeyRetrieval"); // Maybe this isn't the best way of doing it..
    ...
    public MySQLStore() {
        try (Connection conn = getConnection(); Statement stmt = conn.createStatement()) {
            ...
            stmt.executeUpdate( // Increases indexing performance when purging/looking based on time
                "CREATE INDEX IF NOT EXISTS idx_slabbo_transactions_ts ON transactions (" +
                "ts" +
                ")"
            );
    ...
    private Connection getConnection() throws SQLException {
        return DriverManager.getConnection(
            DB_URL,
            Slabbo.getInstance().getConfig().getString("mysql.username"),
            Slabbo.getInstance().getConfig().getString("mysql.password")
        );
    }
    ...
    // @TODO: Use a single Transaction type variable instead?
    // @TODO: Maybe implement this function in all data stores?
    public void addTransaction(String item, String itemNBT, String shop_owner, String shop_user, String buyer, Float price, Double stock, Double quantity) {
        FileConfiguration config = Slabbo.getInstance().getConfig();
        try (Connection conn = getConnection();

             PreparedStatement ps = conn.prepareStatement(
                     "INSERT INTO transactions (item, item_nbt, shop_owner, shop_user shop_user, price, stock, quantity) " +
                             "VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
        ) { // @TODO: Maybe there's some way to make this better with a for loop?
            ps.setString(1, config.getBoolean("transactionLogs.storedInfo.item") ? item: null);
            ps.setString(2, config.getBoolean("transactionLogs.storedInfo.itemNBT") ? itemNBT: null);
            ps.setString(3, config.getBoolean("transactionLogs.storedInfo.shopOwner") ? shop_owner: null);
            ps.setString(4, config.getBoolean("transactionLogs.storedInfo.shopUser") ? shop_user: null);
            ps.setString(5, config.getBoolean("transactionLogs.storedInfo.buyer") ? buyer: null);
            ps.setDouble(6, config.getBoolean("transactionLogs.storedInfo.price") ? price: null);
            ps.setDouble(7, config.getBoolean("transactionLogs.storedInfo.stock") ? stock: null);
            ps.setDouble(8, config.getBoolean("transactionLogs.storedInfo.quantity") ? quantity: null);
            ps.executeUpdate();
        } catch (SQLException e) {
